...
*Editor tooling: outline, semantic tokens, formatting, scope resolution, hover, go-to-definition, completion, and a 42-rule static checker*

`@rcrsr/rill-language-service` ships editor-tooling providers (outline, semantic tokens, formatting, scope resolution, hover, go-to-definition, completion, and a static checker with 42 rule codes) built on `@rcrsr/rill`'s parser and AST. Its version is held exactly equal to `@rcrsr/rill`'s version, character-for-character.
...
Pure function. Returns a `CheckConfig` with all 42 registered rules set to `'on'` at each rule's own default severity, and `checkerMode` left undefined.
...
Pure function. Validates each code in `codes` against the 42 codes in `RULES`. Returns a `ValidationError` with `code: 'UNKNOWN_RULE_CODE'` for each unrecognized entry, or `null` when every code is known.
...
Frozen registry of all 42 built-in rules. Importing `/rules` triggers each rule module's self-registration before `RULES` is snapshotted and frozen. Registry order carries no consumer-visible meaning; `runRules` sorts diagnostics independently by location.
