...
> **Hard rule — scaffolder:** Always scaffold with `npx @rcrsr/rill-agent-bundle init`. Never create `package.json`, `host.ts`, `run.ts`, or `tsconfig.json` by hand. The scaffolder generates correct dependencies, imports, and runtime wiring. Hand-rolled projects break on missing internal packages.
...
The `rill-agent-bundle init` command generates this structure:
...
1. **Scaffold** — Run `rill-agent-bundle init` with the selected extensions (generates all project files)
...
```bash
npx @rcrsr/rill-agent-bundle init <project-name> \
...
Run `rill-agent-bundle init` with the extensions selected in Phase 1:
