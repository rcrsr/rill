...
- **Dict-callable method calls outside pipe-target position:** `$obj.method()` on a dict-held callable raised "Missing argument for parameter ..." when written outside direct pipe-target position, such as inside a ternary branch. `method()` and `-> $obj.method()` both received the piped value in that position. The empty-paren call now receives the piped value in every position. Injection applies only when the call takes zero arguments, a pipe value is in scope, and the callable does not declare an empty parameter list. `$obj.method` without parentheses still evaluates to an unapplied callable, so dispatch tables are unaffected. Consumers will see a behavior change: a dict member whose parameters are all optional with defaults now receives the piped value on an empty-paren call instead of falling back to its defaults. This matches `-> $obj.method()` in direct pipe position. ([#131](https://github.com/rcrsr/rill/issues/131)) ([#134](https://github.com/rcrsr/rill/pull/134))
