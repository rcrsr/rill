...
- **Workflow-artifact ID references removed from shipped source:** 902 internal planning identifiers (`AC-*`, `EC-*`, `IR-*`, `FR-*`, `NFR-*`, and 16 sibling prefixes) are gone from comments and string literals across 90 files under `packages/*/src`. Each comment keeps the fact it stated and drops the unresolvable reference, so `Negative n halts with #INVALID_INPUT (EC-1).` reads `Negative n halts with #INVALID_INPUT.` A new `rill/no-spec-id-reference` rule in `lint-rules/`, scoped to `packages/*/src/**/*.{ts,tsx}` in `.oxlintrc.json`, prevents regression; it scans comments, string literals, template chunks, and JSX text, and is not auto-fixable. rill's own error surface is untouched: `RILL-R010`, `RILL-P007`, `#TYPE_MISMATCH`, and `#INVALID_INPUT` never match. Contributors will see one behavior change: test names under `packages/fiddle/src/**/__tests__` lost their `AC-49: ` style prefixes, so a saved `-t` filter matching those names by string needs updating. No runtime behavior, public API, error ID, or error message changed. ([#139](https://github.com/rcrsr/rill/pull/139))
