...
- **`BARE_CALLABLE_BODY` checker rule:** The language service now warns when a bare builtin or host function name is passed as the body of `seq`, `fan`, `fold`, `filter`, or `acc`, as in `seq(log)`. A bare name in argument position parses as a zero-argument call, so the operator receives that call's result rather than a callable and halts at runtime with a body-must-be-a-closure error. The rule inspects only the body slot, so a zero-argument call in `fold`'s initial-value slot does not fire. The suggested fixes are a closure wrapper, a `$variable` reference, or a namespaced `ns::name` reference. The registry now holds 42 rules. ([#439](https://github.com/rcrsr/rill/issues/439))
...
- **Type-aware dict keys:** A number key, a boolean key, and a string key with the same printed form are now distinct dict entries, and dispatch (`value -> dict`) matches by value and type. Existing string-keyed dicts are unchanged. ([#330](https://github.com/rcrsr/rill/pull/330))
- **Host-stream early-termination disposal guarantee:** A host stream's `dispose` callback now runs exactly once whichever way a collection operator stops consuming the stream — it drains the stream (`seq`, `fan`, `skip`, `stop_when`), stops early (`take`, `break`), or halts — and it runs before the consuming expression returns. A stream stepped by hand with `.first()`/`.next()` is disposed only when a step reaches the end; an abandoned hand-stepped chain is not. Previously `take(n)` that stopped before draining the stream never disposed, leaking one resource per call in long-running hosts. Documented at `createRillStream` in the Host API reference. ([#391](https://github.com/rcrsr/rill/issues/391)) ([#399](https://github.com/rcrsr/rill/pull/399))

### Changed

- **No truthiness:** Logical operators `&&` and `||` now require boolean operands, halting on non-bool values; `.empty` halts on numbers and booleans; and `??` passes present `0` and `false` through instead of vacant. The datetime/duration `.empty` factory is renamed to `.zero`, and `isTruthy` is removed from the public API. ([#250](https://github.com/rcrsr/rill/pull/250))
- **Releases split into two tag namespaces, and tags are checked against the manifest:** `v*` publishes `@rcrsr/rill` with `@rcrsr/rill-language-service` as before; `dev-v*` publishes `@rcrsr/rill-dev` alone, so revving the tooling no longer mints a language version carrying no language change. Both tags must now equal the version in the manifest they release — root for `v*` — or the job fails before publishing, where previously a tag nobody bumped cut a GitHub Release that shipped nothing. ([#152](https://github.com/rcrsr/rill/pull/152))
...
- **Type-system and value corrections:** Several behaviors were corrected to match the language's documented rules and its own principles. `dict[]` no longer compares equal to non-dict values (`dict[] == 5` is `false`). String `.len`, `.head`, `.at`, `.split("")`, and `slice<>` count Unicode code points, so `"😀".len` is `1` and no operation returns a lone surrogate. `string -> number` accepts only clean decimal, float, scientific, and `±Infinity`, rejecting hex/octal/binary and whitespace-padded input. Union type equality is order-insensitive. Dynamic field access on a missing key or a non-dict halts instead of returning a null value (with `??` and `.?` still coalescing and probing). A number or boolean existence-check key returns a bool instead of halting. ([#330](https://github.com/rcrsr/rill/pull/330))

- **Trailing commas rejected in collection literals:** `list[1, 2,]`, `dict[a: 1,]`, `tuple[1,]`, `ordered[a: 1,]`, and the bare `[1, 2,]` forms are now parse errors, matching the grammar and the existing behavior of call and parameter lists, which never accepted one. Multi-line literals are unaffected as long as the last element carries no comma. ([#433](https://github.com/rcrsr/rill/issues/433)) ([#442](https://github.com/rcrsr/rill/pull/442))
- **Line continuation covers every documented token:** A trailing `->`, `=>`, `?`, `!`, `.`, `.!`, `:`, or `^(` before a newline now continues the statement, as the reference's newline table already stated. Only the arithmetic and logical half of that table worked before. ([#422](https://github.com/rcrsr/rill/issues/422)) ([#442](https://github.com/rcrsr/rill/pull/442))

### Fixed

- **Parser, stdlib, and error-message corrections:** Postfix access now chains after a `pass` block, a do-while loop, and a literal-chain `.?field`; a capture target followed by `[i]` or `.field` is a parse error rather than two statements. A non-postfix conditional branch (`true ? 1 + 1 ! 2`) is a parse error naming the fix instead of a misleading runtime negation error. Bare `.?` yields a bool, and `.?` followed by a non-identifier is a parse error. `dict[...$d]` and `ordered[...$o]` spread their entries instead of producing a key named `...`. `sort` halts `#INVALID_INPUT` when the key extractor returns a vacant value. `fan`, `filter`, and `batch` halt on an unknown option key. A leading UTF-8 BOM is skipped, a bare CR inside a single-line string is rejected like LF, a malformed `----` frontmatter closer names the delimiter, and `while (c)` followed by a newline and `{ body }` parses. Trace frames report their real origin instead of always `host`; missing-argument, tuple `slice`/`destruct`, dict-literal `.bogus`, datetime component, and `.iso()` offset messages name the right thing. ([#423](https://github.com/rcrsr/rill/issues/423), [#424](https://github.com/rcrsr/rill/issues/424), [#426](https://github.com/rcrsr/rill/issues/426), [#427](https://github.com/rcrsr/rill/issues/427), [#429](https://github.com/rcrsr/rill/issues/429), [#431](https://github.com/rcrsr/rill/issues/431), [#432](https://github.com/rcrsr/rill/issues/432), [#433](https://github.com/rcrsr/rill/issues/433)) ([#442](https://github.com/rcrsr/rill/pull/442))
- **LLM reference and host-integration docs:** Removed the `seq(log)` bare-callable body form that never parsed as a callable, the nonexistent `atom(name)` call form, and a JSON-parse anti-pattern built on an impossible `string -> :dict` assertion; corrected the `CallableFn` signature in the host integration guide to the named-record form; added the unregistered-atom `#R001` collapse to the cheatsheet. ([#425](https://github.com/rcrsr/rill/issues/425), [#435](https://github.com/rcrsr/rill/issues/435)) ([#442](https://github.com/rcrsr/rill/pull/442))
...
- **Parser and lexer correctness:** Pipe-field chains, escape sequences, and statement continuation now parse. Dangling dots halt; AST spans corrected. ([#331](https://github.com/rcrsr/rill/pull/331))
- **Eight open runtime defects resolved:** `onLog` now defaults to a no-op instead of printing to `console.log`; pass a callback to keep output. `RillError.withContext()` augments an error without mutating the readonly context or source ID of the original. `trackInflight` observes and drops post-dispose promise rejections so they never go unhandled. `EMPTY_BOUND_ARGUMENTS`'s `params` map is typed `ReadonlyMap` so the frozen singleton cannot be mutated through the typed surface. `hydrateFieldDefaults` and `hydrateNested` are unified into one structure walker, removing drift between the two while preserving each caller's missing-field and keep-extras behavior. `bindOrdered()` makes missing-parameter holes explicit at all four call sites instead of an implicit gap. `arrayEquals<T>` replaces the duplicated index-loop comparators. A bare `.!` probe no longer allocates a status object it discards. ([#242](https://github.com/rcrsr/rill/pull/242))
- **12 language-service defects resolved:** `getChildren` now visits property-access, dict-key, and type-arg children that `astChildren` already covers in core, closing the parity gap between the two traversals. `LOOP_ACCUMULATOR` looks up the loop condition's precomputed `referenceStart`/`referenceEnd` window via `context.facts.bySubtree.get(loop.condition)` and slices `referenceLog`, instead of re-walking the condition with its own `collectVariableReferences`. `use-default-operator`'s `hasExistenceCheck` now recurses via the exported `walkAst` instead of an `Object.keys` walk, so it no longer risks a `RangeError` on deeply nested ASTs. `semanticTokens` replaces the linear `collectTypeNameSpans`/`isSpanWithin` scan with a sorted, binary-searchable `buildTypeNameCoverage`/`isCoveredByTypeNameSpan`. `runRules` buckets enabled rules by `NodeType` up front in `buildEnabledRuleBuckets`, replacing a per-node `nodeTypes.includes` scan across all registered rules with a single `Map` lookup. `getHover` only calls `hoverForClosureCall` for `capture` bindings, since `introspectHandlerFromAST` never matches other binding kinds, and reads `declaredType` directly from `resolveScopeAt`'s `Binding` instead of re-walking the AST via the removed `findDeclaredTypeRef`. `findBracketAccessSegmentAt` scopes its `walkAst` call to the single top-level statement containing the offset via the new `findContainingStatement`, instead of walking the whole script. `resolveScopeAt` and `findVisibleBinding` share one AST walk through the new `computeScopeState`, which also determines closure-body enclosure, removing the second full-AST pass `isInsideClosureBody` used to require. `SPACING_OPERATOR` drops its inert `Capture`/`checkCaptureSpacing` branch, documented as dead since `CaptureNode.span` no longer covers the `=>` token, and `checkOperatorSpacing` scans the masked span text directly instead of building a `RegExp` per node. `implicit-dollar-closure` and `implicit-dollar-function` share their diagnostic construction through the new `bareDollarSingleArgDiagnostic` helper. `types-assertion`'s `buildFix` searches for the type-annotation colon starting after the operand's own span end via the new `advancePosition`, instead of from the start of the assertion text, so a colon inside a string-literal operand is no longer mistaken for the type separator. ([#243](https://github.com/rcrsr/rill/pull/243))
- **Language service checker and formatter correctness:** Fixed ten checker rule false-positives (`CLOSURE_BARE_DOLLAR` on the dict-bound closure idiom, `ATOM_UNREGISTERED` on pre-registered runtime atoms, `USE_DEFAULT_OPERATOR` on non-verbose-default conditionals, `LOOP_OUTER_CAPTURE`/`AVOID_REASSIGNMENT` on sibling loops reusing a local name, `THROWAWAY_CAPTURE` on dynamic field-key references, `SPACING_OPERATOR` on operator-like text inside comments), a mis-scoped `NAMING_SNAKE_CASE` auto-fix that could rewrite a reference belonging to a different declaration, an unparseable `USE_EMPTY_METHOD` suggestion, a `STATUS_PROBE_NO_FIELD` message stating the wrong semantics for bare `.!`, and a formatter bug that trimmed trailing whitespace inside triple-quoted string literals. ([#333](https://github.com/rcrsr/rill/pull/333))
- **Fiddle playground:** Resolved 12 defects including reachable code-share functionality, all examples accessible from the dropdown, split-divider touch interaction and ARIA bounds corrected, React StrictMode compliance, and per-keystroke overhead eliminated. ([#246](https://github.com/rcrsr/rill/pull/246))
- **Core defect fixes:** Invalid regex patterns in string methods now halt with catchable #INVALID_INPUT instead of silently returning wrong data. Injected timezone, clock, and scheduler now consistently apply in nested bodies. Consumers will see a behavior change: a malformed pattern passed to `.replace`, `.replace_all`, `.match`, or `.is_match` previously ran to completion, returning the original string, `{}`, or `false`; it now halts with `#INVALID_INPUT` and must be wrapped in `guard`, `retry`, or resolved with `??` to recover. ([#247](https://github.com/rcrsr/rill/pull/247))
...
- **Parser and runtime error-halting:** Eight defects in parser, runtime, types, and stdlib that previously crashed or silently misparsed now halt with registered error IDs. Consumers will see a behavior change on each of these paths: dict literals previously let a script forge a callable, type, or datetime value by writing one of the eight runtime brand keys (`__type`, `__rill_atom`, `__rill_tuple`, `__rill_vector`, `__rill_datetime`, `__rill_duration`, `__rill_ordered`, `__rill_type`) as a key; all four dict key-evaluation paths now reject these keys with `RILL-R002`. An else-less conditional whose false branch has no bound `$` previously leaked a raw `null`; it now halts `RILL-R005`, matching bare `pass`. `log()` with no arguments previously returned `null`; it now halts `RILL-R044`. `tuple -> list` on mixed element types previously produced a list that could crash later in `inferStructure`; it now halts `RILL-R002` with the same message as the list literal. `take` and `skip` over a host stream previously skipped the chunk-type check that `seq` and `fan` already enforce; they now halt `#TYPE_MISMATCH` on the first divergent chunk. A malformed `SchemeResolver` result — not an object, with no valid `kind`, or `kind: 'value'` with no `value` — previously fell through to the source branch or returned `undefined`; it now halts `RILL-R056`. `isDictStart` previously miscounted nesting depth for `dict[`, `list[`, `tuple[`, and `ordered[` openers, misparsing a collection literal inside a multi-key dict key; it now counts all four openers and parses correctly. `.match`, `.replace`, `.replace_all`, and `.is_match` now compile their pattern with the `u` (unicode) flag so astral characters match as one code point instead of yielding lone surrogates; unicode-mode regex compilation is stricter about escape sequences than the previous behavior, so a pattern with a superfluous backslash escape on an ordinary character — previously accepted silently as a literal — now throws a `SyntaxError` surfaced as `#INVALID_INPUT`. ([#397](https://github.com/rcrsr/rill/pull/397))
- **Parser and type-system correctness:** Eleven defects fixed in closure and conditional newline handling, type validation, vacancy coalescing, signature matching, and parameter defaults. ([#398](https://github.com/rcrsr/rill/pull/398))
- **`toNative()` typed dict keys:** No longer collapses number and boolean dict keys onto their string form; typed keys are preserved under a reserved `__rill_typed_keys` field. All-string dicts are unchanged. `dict["__rill_typed_keys": ...]` now halts as a reserved brand key. ([#398](https://github.com/rcrsr/rill/pull/398))
- **Parser, runtime, and stdlib correctness:** Resolved 12 validated defects across the lexer, runtime evaluator, and standard library. Highlights: `fan`/`filter` with a fractional `concurrency` in (0,1) exhausted the heap instead of halting; `take(n)` on a host stream leaked the source by never disposing on early stop; `.first()` and `sort` on a fresh host stream iterated the stream internal fields; `.at` out-of-bounds and `.head`/`.tail` on an empty list escaped `guard`; a `return` inside a conditional or `do…while` condition was swallowed into the condition value; `$d.a()` on a non-callable field returned the field instead of halting; dynamic access `$d.$k`/`$d.(expr)` with a number or boolean key halted on a typed-key dict; `"hello" -> $.upper` failed in pipe-target position; identifiers named after `Object.prototype` members (`toString`, `valueOf`, `constructor`) lexed as a JS function instead of an identifier; the `RILL_R012` timeout atom was unregistered, so `.!code` reported `R001`; `.^description` returned `dict[]` instead of `""` when absent; and `"ok" -> atom` and `invalidate({ code: 'ok' })` minted the reserved `#ok` sentinel. ([#336](https://github.com/rcrsr/rill/issues/336), [#354](https://github.com/rcrsr/rill/issues/354), [#367](https://github.com/rcrsr/rill/issues/367), [#368](https://github.com/rcrsr/rill/issues/368), [#370](https://github.com/rcrsr/rill/issues/370), [#371](https://github.com/rcrsr/rill/issues/371), [#372](https://github.com/rcrsr/rill/issues/372), [#373](https://github.com/rcrsr/rill/issues/373), [#375](https://github.com/rcrsr/rill/issues/375), [#388](https://github.com/rcrsr/rill/issues/388), [#389](https://github.com/rcrsr/rill/issues/389), [#391](https://github.com/rcrsr/rill/issues/391)) ([#399](https://github.com/rcrsr/rill/pull/399))
- **Lexer, parser, and stdlib correctness:** Resolved 12 validated defects. Lexer: `$1`, `1_000`, and `1abc` now halt at lex time with `RILL-L003` naming the offending token instead of splitting into two tokens and surfacing a misleading runtime error; a `{` or `}` inside a string literal nested in an interpolation no longer desyncs the brace scan; a triple-quoted string strips a leading `\r\n` as well as `\n`. Parser: a malformed atom such as `#Foo` in expression position now reports `RILL-P004` in both strict and recovery modes instead of silently executing; closure-parameter and structural-field defaults accept negative numbers, `list[]`/`dict[]`/`tuple[]`/`ordered[]` literals, and atoms; `slice<1::2>` parses. Runtime: spreading an empty tuple or dict into a single untyped-parameter closure reports `Missing argument` instead of `Undefined variable`. Stdlib: `.replace` and `.replace_all` insert the replacement literally instead of expanding JavaScript `$`-substitution patterns; `.pad_start` and `.pad_end` measure length in code points, matching `.len`; `duration.multiply` halts on fractional months or a non-finite result instead of producing a duration the constructor would reject; `batch` halts `#TYPE_MISMATCH` on a non-dict options argument or a non-bool `drop_partial`; `datetime("…T24:00:00Z")` halts `#INVALID_INPUT` instead of rolling to the next day. ([#356](https://github.com/rcrsr/rill/issues/356), [#357](https://github.com/rcrsr/rill/issues/357), [#358](https://github.com/rcrsr/rill/issues/358), [#360](https://github.com/rcrsr/rill/issues/360), [#361](https://github.com/rcrsr/rill/issues/361), [#363](https://github.com/rcrsr/rill/issues/363), [#364](https://github.com/rcrsr/rill/issues/364), [#365](https://github.com/rcrsr/rill/issues/365), [#369](https://github.com/rcrsr/rill/issues/369), [#376](https://github.com/rcrsr/rill/issues/376), [#386](https://github.com/rcrsr/rill/issues/386), [#394](https://github.com/rcrsr/rill/issues/394)) ([#400](https://github.com/rcrsr/rill/pull/400))
- **Parser, runtime, stdlib, and docs correctness:** Resolved 11 validated defects. Parser: keyword names (`true`, `false`, `break`, `return`, `yield`, `pass`, `assert`, `error`, `guard`, `retry`, `while`, `do`) now parse after `$` as variable names, closure parameters, capture targets, closure calls, destruct patterns, `.$field`, `use<$x>`, and `:$type`; `ordered[-1: "x"]` parses instead of misfiring the missing-key guard on a leading `-`; an interpolation error inside a double-quoted string reports the correct source column instead of drifting left by one per preceding escape sequence. Runtime: any arithmetic result that is not finite now halts `#INVALID_INPUT` instead of silently producing `NaN`/`Infinity`, and `"Infinity"`/`"-Infinity" -> number` halt the same way `"NaN" -> number` already did; all eight dict method names (`len`, `first`, `empty`, `eq`, `ne`, `keys`, `values`, `entries`) are reserved as dict keys instead of three, closing a divergence where dot access resolved the method and bracket access resolved the field on the same dict; `getFunctions` reports a script closure's declared return type and reads the `^("description")` shorthand instead of hardcoding `returnType: 'any'` and an empty description. Stdlib: `.iso` pads a negative year's digits instead of its sign and emits ISO 8601 expanded-year notation, so years outside `0..9999` round-trip through `datetime()`. Docs: corrected the documented `enumerate` signature, the `ordered-element` grammar rule, dict `sort`'s default-key behavior, and which halts `guard` catches. ([#359](https://github.com/rcrsr/rill/issues/359), [#380](https://github.com/rcrsr/rill/issues/380), [#382](https://github.com/rcrsr/rill/issues/382), [#387](https://github.com/rcrsr/rill/issues/387), [#377](https://github.com/rcrsr/rill/issues/377), [#383](https://github.com/rcrsr/rill/issues/383), [#385](https://github.com/rcrsr/rill/issues/385), [#393](https://github.com/rcrsr/rill/issues/393), [#381](https://github.com/rcrsr/rill/issues/381), [#384](https://github.com/rcrsr/rill/issues/384), [#395](https://github.com/rcrsr/rill/issues/395)) ([#401](https://github.com/rcrsr/rill/pull/401))
- **Runtime, parser, stdlib, and host-boundary correctness:** Resolved 6 validated defects. Parser: AST node spans now end at the last consumed token instead of the next unconsumed one (~35 sites; hover/selection in the language service no longer bleed onto the next line). Runtime: dict iteration and formatting use one canonical key order (string keys lexical, then number keys ascending, then booleans false before true) so equal dicts give equal `.keys`/`.values`/`.entries`/`seq`/`enumerate`/`-> string`; host function return values are validated at the call boundary and `undefined`, `null`, `Symbol`, `Date`, `Map`, `Set`, and nested leaks halt with the new `RILL-R085`; `generateManifest` always emits parseable rill (explicit return type, defaults never emitted). Stdlib: `createTestContext` throws `ExtensionBindingError` at construction for unparseable bindings instead of a late `RILL-R056`. ([#378](https://github.com/rcrsr/rill/issues/378), [#392](https://github.com/rcrsr/rill/issues/392), [#390](https://github.com/rcrsr/rill/issues/390), [#101](https://github.com/rcrsr/rill/issues/101), [#379](https://github.com/rcrsr/rill/issues/379), [#374](https://github.com/rcrsr/rill/issues/374)) ([#402](https://github.com/rcrsr/rill/pull/402))
- Host functions returning `NaN` or `±Infinity` now halt with a catchable `#INVALID_INPUT` error recoverable by `guard` and `??`, instead of flowing into the runtime.
- Closure-signature literals accept a zero-parameter form (`||:ret`) and per-parameter `^(...)` annotations; `generateManifest` emits every entry in this form and no longer falls back to a placeholder closure body.
- **Postfix bracket-index access:** Bracket-index access `[i]` after a literal or a grouped (parenthesized) expression now works as postfix indexing. ([#408](https://github.com/rcrsr/rill/pull/408))
- **Postfix indexing on more targets:** Indexing `[i]` now works on tuples (`tuple[1,"a"][1]`), on `ordered` values by key and position, and on the result of a bare operator or host-call pipe target (`list[3,1,2] -> sort[0]`). `ordered` values also answer `.field`, `.keys`, `.values`, `.entries`, `.len`, and `.?field` from their entries instead of leaking internal fields, and tuples support `.at(i)`. An existence check (`.?field`) or a `:type` / `:?type` assertion may now follow any postfix operator, so `list[dict[a:1]][0].?a` and `list[1,2,3][0]:number` parse. Indexing an iterator directly (`range(0,5)[2]`) now halts with `RILL-R002` "Cannot index iterator" instead of a misleading "Undefined dict key". ([#403](https://github.com/rcrsr/rill/issues/403), [#404](https://github.com/rcrsr/rill/issues/404), [#405](https://github.com/rcrsr/rill/issues/405), [#406](https://github.com/rcrsr/rill/issues/406), [#407](https://github.com/rcrsr/rill/issues/407)) ([#409](https://github.com/rcrsr/rill/pull/409))
- **Bare `assert` no longer leaks `null`:** A statement-form `assert` that passes with no bound `$` (for example as the last statement of a parameterless closure) now halts `RILL-R005`, the same as bare `pass`, instead of returning a raw JS `null` that then converted, interpolated, and serialized as a value. ([#410](https://github.com/rcrsr/rill/issues/410)) ([#436](https://github.com/rcrsr/rill/pull/436))
- **Recursion is bounded:** Closure invocation now carries a call-depth ceiling, `maxCallDepth` in `RuntimeOptions` (default 1000), alongside the loop and iterator ceilings. Exceeding it halts with a fatal `RILL-R010` instead of exhausting the host heap or stack. Each call level also yields one microtask, so a recursion path with no suspension point between levels can no longer overflow the native stack before the ceiling fires. ([#411](https://github.com/rcrsr/rill/issues/411)) ([#436](https://github.com/rcrsr/rill/pull/436))
- **`break` outside a loop halts:** A `break` inside `fan`, `filter`, `fold`, `sort`, a `retry` body, or at the top level now halts with a fatal `RILL-R002` "break not supported in ..." instead of escaping `execute()` as a raw `BreakSignal`, and `guard` does not recover it. `seq`, `acc`, and `while` still consume `break` as before. ([#412](https://github.com/rcrsr/rill/issues/412)) ([#436](https://github.com/rcrsr/rill/pull/436))
- **`:?any` is true:** The type probe `value:?any` now returns `true` for every value, matching the `:any` assertion. It previously always returned `false`. ([#413](https://github.com/rcrsr/rill/issues/413)) ([#436](https://github.com/rcrsr/rill/pull/436))
- **Branded values no longer fall into the dict path:** Atoms, type values, datetimes, and durations now halt like numbers and bools when used as the receiver of `.keys`, `.values`, `.entries`, or `.first`, or as the input of `seq`, `fan`, `fold`, `filter`, `sort`, `take`, and `skip`, instead of leaking internal fields such as `__rill_atom` or `__rill_datetime`. `ordered` values halt on `.first`. ([#414](https://github.com/rcrsr/rill/issues/414)) ([#436](https://github.com/rcrsr/rill/pull/436))
- **Host-boundary halt and error surfacing:** Halts raised at runtime host boundaries now carry an informative message, a resolvable error ID, and a source location where derivable, instead of escaping as a bare `RuntimeHaltSignal` with the literal message "runtime halt". Synchronous host-function throws and stream `resolve()` results are validated at the boundary instead of leaking raw errors. ([#415](https://github.com/rcrsr/rill/issues/415), [#416](https://github.com/rcrsr/rill/issues/416), [#420](https://github.com/rcrsr/rill/issues/420), [#421](https://github.com/rcrsr/rill/issues/421), [#428](https://github.com/rcrsr/rill/issues/428), [#430](https://github.com/rcrsr/rill/issues/430)) ([#437](https://github.com/rcrsr/rill/pull/437))
- **Host stream consumption:** `take(n)` no longer pulls one chunk past the cap, `seq`/`acc` consume stream and iterator inputs lazily so `break` bounds pulls from an unbounded source, chunk validation now catches `NaN`, `null`, callable, and `undefined` chunk values on stream input to `seq`, `acc`, `fold`, `fan`, `filter`, `take`, and `skip`, and a mid-stream host throw disposes the stream and halts instead of being silently swallowed. ([#417](https://github.com/rcrsr/rill/issues/417), [#418](https://github.com/rcrsr/rill/issues/418), [#419](https://github.com/rcrsr/rill/issues/419), [#440](https://github.com/rcrsr/rill/issues/440)) ([#443](https://github.com/rcrsr/rill/pull/443))
- **Host function typed-key access:** Host functions now read typed dict keys via the new `getTypedKeyEntries` export, and stream pending events carry an explicit discriminator. Integration documentation clarifies named-argument access for typed functions. ([#444](https://github.com/rcrsr/rill/pull/444))
...
- **Workflow-artifact ID references removed from shipped source:** 900 internal planning identifiers (`AC-*`, `EC-*`, `IR-*`, `FR-*`, `NFR-*`, and 16 sibling prefixes) are gone from comments and string literals across 87 files under `packages/*/src`. Each comment keeps the fact it stated and drops the reference to the planning document. For example, `Negative n halts with #INVALID_INPUT (see plan).` becomes `Negative n halts with #INVALID_INPUT.` A new `rill/no-spec-id-reference` rule in `lint-rules/`, scoped to `packages/*/src/**/*.{ts,tsx}` in `.oxlintrc.json`, prevents regression; it scans comments, string literals, template chunks, and JSX text, and is not auto-fixable. The rule cannot reach `.rill` fixtures, since oxlint has no `.rill` parser, so 8 of the 10 fixtures under `packages/fiddle/src/lib/__tests__/fixtures/error-parity/` were scrubbed by hand. rill's own error surface is untouched: `RILL-R010`, `RILL-P007`, `#TYPE_MISMATCH`, and `#INVALID_INPUT` never match. Contributors will see one behavior change: test names under `packages/fiddle/src/**/__tests__` lost their numbered ID prefixes, so a saved `-t` filter matching those names by string needs updating. No runtime behavior, public API, error ID, or error message changed. ([#139](https://github.com/rcrsr/rill/pull/139))
...
- **Reserved-word member access:** All 12 reserved words (`true`, `false`, `break`, `return`, `yield`, `pass`, `assert`, `error`, `guard`, `retry`, `while`, `do`) now parse as member names after `.` or `.?`, so `$app.error("x")` and `$obj.while` work instead of halting with a message naming a statement the script never used. This includes the four words that also open compound forms, so `$d.retry<10` reads the `retry` member and compares it rather than starting a `retry<...>` block. Bare dict keys are unchanged: `dict[error: 1]` is still invalid and `dict["error": 1]` remains the spelling. ([#136](https://github.com/rcrsr/rill/pull/136))
- **Whitespace-trap documentation:** `docs/llm/anti-patterns.txt` labelled `$obj. field` an error and `$x->.upper` invalid. Both parse and run; only `list [1, 2, 3]` halts. The section now separates the one parse error from the info-level checker diagnostics. ([#136](https://github.com/rcrsr/rill/pull/136))
...
- **Dict-callable method calls outside pipe-target position:** `$obj.method()` on a dict-held callable raised "Missing argument for parameter ..." when written outside direct pipe-target position, such as inside a ternary branch. The flat host call `method()` and the direct pipe target `-> $obj.method()` already received the piped value; only the bare `$obj.method()` form was broken. The empty-paren call now receives the piped value in every position. Injection applies only when the call takes zero arguments, a pipe value is in scope, and the callable does not declare an empty parameter list. `$obj.method` without parentheses still evaluates to an unapplied callable, so dispatch tables are unaffected. Consumers will see a behavior change: a dict member whose parameters are all optional with defaults now receives the piped value on an empty-paren call. It no longer falls back to its defaults. This matches `-> $obj.method()` in direct pipe position. ([#131](https://github.com/rcrsr/rill/issues/131)) ([#134](https://github.com/rcrsr/rill/pull/134))
- **`$`-binding documentation for conditionals:** The conditional row in `docs/ref-language.md`, `docs/topic-variables.md`, `docs/llm/cheatsheet.txt`, and `docs/ref-llms-full.txt` read `cond ? { }` and described the bound value as the "tested value," which implied block-only binding. Both branches bind `$`, block or bare, to the pipe value in effect when the conditional was entered, not to the condition's result. ([#131](https://github.com/rcrsr/rill/issues/131)) ([#134](https://github.com/rcrsr/rill/pull/134))
