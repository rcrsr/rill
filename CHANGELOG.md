...
- **`@rcrsr/rill-dev`:** New published package carrying the ecosystem's shared development assets: the `rill-check-standards` conformance checker, the custom oxlint rules (`rill/no-duplicate-error-id`, `rill/no-spec-id-reference`) exposed at `@rcrsr/rill-dev/lint-rules`, and `REPO-STANDARDS.md` itself. The checker decides 55 elements from the repository and 6 more from the GitHub API under `--remote`; elements needing human judgement are reported as unchecked and counted separately rather than passed silently, so a green run never reads as full conformance. Sibling repositories consume it as a devDependency and upgrade by version, replacing the copied `dev/` directory and its drift check. Its version floats independently of `@rcrsr/rill`. ([#152](https://github.com/rcrsr/rill/pull/152))
...
- **Release tags are verified against the manifest they release:** `release.yml` reconciled the manifests against each other but never against the tag. Tagging a version nobody bumped published nothing — every version was already on the registry, so the loop skipped everything and reported success — and the job still cut a GitHub Release for a version that shipped nowhere. A `v*` tag must now equal the root `package.json` version and a `dev-v*` tag the `packages/dev` version, or the job fails before publishing. The `v*` gate reads root rather than `packages/core` because root increments on every release while core increments only when core changes; `v0.18.1` shipped root `0.18.1` against core `0.18.0`, and gating on core would have failed it. ([#152](https://github.com/rcrsr/rill/pull/152))
- **Releases split into two tag namespaces:** `.github/workflows/release.yml` now triggers on `dev-v*` alongside `v*` and derives its publish set from the tag. `v*` publishes `@rcrsr/rill` with `@rcrsr/rill-language-service` as before; `dev-v*` publishes `@rcrsr/rill-dev` alone. Because that package versions independently of the language, a shared tag namespace would have forced a language version carrying no language change every time the tooling was revved, and that release would have taken the repository's "Latest release" badge. A `dev-v*` release is created with `--latest=false`, and its notes link the package changelog rather than generating notes across the intervening language commits. ([#152](https://github.com/rcrsr/rill/pull/152))
