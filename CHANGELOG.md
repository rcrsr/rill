...
- **Bare `assert` no longer leaks `null`:** A statement-form `assert` that passes with no bound `$` (for example as the last statement of a parameterless closure) now halts `RILL-R005`, the same as bare `pass`, instead of returning a raw JS `null` that then converted, interpolated, and serialized as a value. ([#410](https://github.com/rcrsr/rill/issues/410))
- **Recursion is bounded:** Closure invocation now carries a call-depth ceiling, `maxCallDepth` in `RuntimeOptions` (default 1000), alongside the loop and iterator ceilings. Exceeding it halts with a fatal `RILL-R010` instead of exhausting the host heap or stack. Each call level also yields one microtask, so a recursion path with no suspension point between levels can no longer overflow the native stack before the ceiling fires. ([#411](https://github.com/rcrsr/rill/issues/411))
- **`break` outside a loop halts:** A `break` inside `fan`, `filter`, `fold`, `sort`, a `retry` body, or at the top level now halts with a fatal `RILL-R002` "break not supported in ..." instead of escaping `execute()` as a raw `BreakSignal`, and `guard` does not recover it. `seq`, `acc`, and `while` still consume `break` as before. ([#412](https://github.com/rcrsr/rill/issues/412))
- **`:?any` is true:** The type probe `value:?any` now returns `true` for every value, matching the `:any` assertion. It previously always returned `false`. ([#413](https://github.com/rcrsr/rill/issues/413))
- **Branded values no longer fall into the dict path:** Atoms, type values, datetimes, and durations now halt like numbers and bools when used as the receiver of `.keys`, `.values`, `.entries`, or `.first`, or as the input of `seq`, `fan`, `fold`, `filter`, `sort`, `take`, and `skip`, instead of leaking internal fields such as `__rill_atom` or `__rill_datetime`. `ordered` values halt on `.first`. ([#414](https://github.com/rcrsr/rill/issues/414))
