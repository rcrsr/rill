...
- **Host-stream early-termination disposal guarantee:** A host stream's `dispose` callback now runs exactly once whichever way a collection operator stops consuming the stream — it drains the stream, stops early (`take`, `skip`, `stop_when`, `break`), or halts — and it runs before the consuming expression returns. A stream stepped by hand with `.first()`/`.next()` is disposed only when a step reaches the end; an abandoned hand-stepped chain is not. Previously `take(n)` that stopped before draining the stream never disposed, leaking one resource per call in long-running hosts. Documented at `createRillStream` in the Host API reference. ([#391](https://github.com/rcrsr/rill/issues/391)) ([#399](https://github.com/rcrsr/rill/pull/399))
