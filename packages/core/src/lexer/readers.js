...
import { isDigit, isIdentifierChar, isIdentifierStart, makeToken, } from './helpers.js';
import { KEYWORDS } from './operators.js';
import { advance, currentLocation, isAtEnd, peek, } from './state.js';
...
function processEscape(state) {
    ...
    switch (escaped) {
        ...
        case '"':
            return '"';
        default:
            throw new LexerError('RILL-L005', `Invalid escape sequence: \\${escaped}`, currentLocation(state));
    }
}
...
export function readTripleQuoteString(state) {
    const start = currentLocation(state);
        ...
        if (peek(state) === '{') {
            ...
            while (!isAtEnd(state) && braceDepth > 0) {
                ...
                if (peek(state) === '"' &&
                    peek(state, 1) === '"' &&
                    peek(state, 2) === '"') {
                    throw new LexerError('RILL-L005', 'Triple-quotes not allowed in interpolation', currentLocation(state));
                }
