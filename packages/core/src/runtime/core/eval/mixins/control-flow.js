...
function createControlFlowMixin(Base) {
    return class ControlFlowEvaluator extends Base {
        ...
        async evaluateConditional(node) {
            ...
            if (node.condition) {
                ...
                if (typeof conditionValue !== 'boolean') {
                    throw RuntimeError.fromNode('RILL-R002', `Conditional expression must be boolean, got ${inferType(conditionValue)}`, node);
                }
                ...
                if (typeof this.ctx.pipeValue !== 'boolean') {
                    throw RuntimeError.fromNode('RILL-R002', `Piped conditional requires boolean, got ${inferType(this.ctx.pipeValue)}`, node);
                }
        ...
        async evaluateWhileLoop(node) {
            ...
            // Condition must be boolean
            if (typeof conditionValue !== 'boolean') {
                throw RuntimeError.fromNode('RILL-R002', `While loop condition must be boolean, got ${typeof conditionValue}`, node);
            }
            ...
            try {
                ...
                while (conditionResult) {
                    ...
                    if (iterCount > maxIter) {
                        throw new RuntimeError('RILL-R010', `While loop exceeded ${maxIter} iterations`, this.getNodeLocation(node), { limit: maxIter, iterations: iterCount });
                    }
                    ...
                    if (typeof nextCondition !== 'boolean') {
                        throw RuntimeError.fromNode('RILL-R002', `While loop condition must be boolean, got ${typeof nextCondition}`, node);
                    }
        ...
        async evaluateDoWhileLoop(node) {
            ...
            try {
                ...
                while (shouldContinue) {
                    ...
                    // Condition must be boolean
                    if (typeof conditionValue !== 'boolean') {
                        throw RuntimeError.fromNode('RILL-R002', `Do-while condition must be boolean, got ${inferType(conditionValue)}`, node);
                    }
         ...
         */
        async evaluateAssert(node, input) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const conditionResult = await this.evaluateExpression(node.condition);
            // Restore the pipe value (condition evaluation may have changed it)
            this.ctx.pipeValue = savedPipeValue;
            // Condition must be boolean
            if (typeof conditionResult !== 'boolean') {
                throw RuntimeError.fromNode('RILL-R002', `assert requires boolean condition, got ${inferType(conditionResult)}`, node);
            }
            ...
            if (!conditionResult) {
                ...
                if (node.message) {
                    ...
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const messageValue = await this.evaluateString(node.message);
        ...
        async evaluateError(node, input) {
            ...
            if (node.message) {
                ...
                throw RuntimeError.fromNode('RILL-R002', 'error statement requires string message', node);
            }
            // Message must be string
            if (typeof messageValue !== 'string') {
                throw RuntimeError.fromNode('RILL-R002', `error statement requires string message, got ${inferType(messageValue)}`, node);
            }
