 ...
 */
export async function evaluateDestructure(
  s: EvalState,
  ...
  if (isKeyPattern) {
    ...
    for (let i = 0; i < node.elements.length; i++) {
      ...
      if (elem.kind === 'nested' && elem.nested) {
        await evaluateDestructure(s, elem.nested, value);
        continue;
      }
...
export async function evaluateSlice(
  ...
  const startBound = node.start
    ? await evaluateSliceBound(s, node.start, node.span.start)
    : null;
  const stopBound = node.stop
    ? await evaluateSliceBound(s, node.stop, node.span.start)
    : null;
  const stepBound = node.step
    ? await evaluateSliceBound(s, node.step, node.span.start)
    : null;

  if (isList) {
    return applySlice(s, input, input.length, startBound, stopBound, stepBound);
  }
  return applySlice(s, input, input.length, startBound, stopBound, stepBound);
}
...
export async function evaluateSliceBound(
  ...
  if (bound === null) {
    throwCatchableHostHalt(
      { location, sourceId: s.ctx.sourceId, fn: 'evaluateSliceBound' },
      ERROR_ATOMS[ERROR_IDS.RILL_R002],
      'Slice bound is null'
    );
  }
 ...
 */
export async function evaluateDestruct(
  s: EvalState,
  node: DestructNode,
  input: RillValue
): Promise<RillValue> {
 ...
 */
export async function evaluateCollectionLiteral(
  s: EvalState,
  node:
    | ListLiteralNode
    | DictLiteralNode
  ...
  switch (node.type) {
    case 'ListLiteral': {
      const listItems = await evaluateListLiteralElements(s, node.elements);
    ...
    case 'TupleLiteral': {
      ...
      const items = await evaluateListLiteralElements(s, node.elements);
      return createTuple(items);
    }

    case 'DictLiteral': {
      const result: Record<string, RillValue> = {};
      for (const [key, value] of await evaluateDictLiteralEntries(
        s,
        node.entries
      )) {
        result[key] = value;
...
>(Base: TBase) {
  return class ExtractionEvaluator extends Base {
    ...
    evaluateDestructure(
      ...
      return evaluateDestructure(this as unknown as EvalState, node, input);
    }
    ...
    evaluateSlice(node: SliceNode, input: RillValue): Promise<RillValue> {
      return evaluateSlice(this as unknown as EvalState, node, input);
    }
    ...
    evaluateSliceBound(
      ...
      return evaluateSliceBound(this as unknown as EvalState, bound, location);
    }
    ...
    evaluateDestruct(node: DestructNode, input: RillValue): Promise<RillValue> {
      return evaluateDestruct(this as unknown as EvalState, node, input);
    }
    ...
    evaluateCollectionLiteral(
      ...
      return evaluateCollectionLiteral(this as unknown as EvalState, node);
    }
     ...
     * Spread entries (...$other) expand inline (dict keys merged).
     */
    evaluateDictLiteralEntries(
      entries: DictEntryNode[]
    ): Promise<[string, RillValue][]> {
      return evaluateDictLiteralEntries(this as unknown as EvalState, entries);
    }
  };
}
