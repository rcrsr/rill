        ...
        async expandIterator(iterator, node, limit = DEFAULT_MAX_ITERATIONS) {
            ...
            while (!current['done'] && count < limit) {
                ...
                if (nextClosure === undefined || !isCallable(nextClosure)) {
                    throw new RuntimeError('RILL-R002', 'Iterator .next must be a closure', node.span.start);
                }
                ...
                if (typeof nextIterator !== 'object' || nextIterator === null) {
                    throw new RuntimeError('RILL-R002', 'Iterator .next must return iterator', node.span.start);
                }
                current = nextIterator;
            }
            if (count >= limit) {
                throw new RuntimeError('RILL-R010', `Iterator expansion exceeded ${limit} iterations`, node.span.start, { limit, iterations: count });
            }
            return elements;
        }
        /**
        ...
        async evaluateIteratorBody(body, element, accumulator) {
            switch (body.type) {
                ...
                case 'Variable': {
                    ...
                    if (!varValue) {
                        throw new RuntimeError('RILL-R005', `Undefined variable: $${body.name}`, body.span.start);
                    }
                    if (!isCallable(varValue)) {
                        throw new RuntimeError('RILL-R002', `Collection body variable must be callable, got ${inferType(varValue)}`, body.span.start);
                    }
                    const args = [element];
                    if (accumulator !== null &&
                ...
                case 'Spread': {
                    ...
                    throw new RuntimeError('RILL-R002', `Spread requires list or dict, got ${inferType(element)}`, body.span.start);
                }
                case 'HostCall': {
                    // Host function call: invoke with element as first arg
                    const fn = this.ctx.functions.get(body.name);
                    if (!fn) {
                        throw new RuntimeError('RILL-R006', `Unknown function: ${body.name}`, body.span.start, { functionName: body.name });
                    }
                    const args = [element];
                    if (accumulator !== null) {
                        args.push(accumulator);
                ...
                default: {
                    ...
                    throw new RuntimeError('RILL-R002', `Unsupported iterator body type: ${unknownBody.type}`, unknownBody.span.start);
                }
            }
        }
        /**
        ...
        async evaluateFold(node, input) {
            ...
            if (node.accumulator) {
                ...
                if (lastParam?.defaultValue) {
                    ...
                    throw new RuntimeError('RILL-R002', 'Fold requires accumulator: use |x, acc = init| or fold(init) { }', node.span.start);
                }
            }
                ...
                if (!varValue || !isCallable(varValue) || varValue.kind !== 'script') {
                    throw new RuntimeError('RILL-R002', 'Fold variable must be a script closure with accumulator parameter', node.span.start);
                }
                ...
                if (lastParam && lastParam.defaultValue !== null) {
                    ...
                    throw new RuntimeError('RILL-R002', 'Fold closure must have accumulator parameter with default value', node.span.start);
                }
            }
            else {
                throw new RuntimeError('RILL-R002', 'Fold requires accumulator: use |x, acc = init| or fold(init) { }', node.span.start);
        ...
        async evaluateFilter(node, input) {
            ...
            const predicatePromises = elements.map(async (element) => {
                ...
                if (typeof result !== 'boolean') {
                    throw new RuntimeError('RILL-R002', `Filter predicate must return boolean, got ${inferType(result)}`, node.span.start);
                }
