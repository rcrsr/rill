    ...
    return class VariablesEvaluator extends Base {
        ...
        setVariable(name, value, explicitType, location) {
            ...
            if (explicitType !== undefined && explicitType !== valueType) {
                throw new RuntimeError('RILL-R001', `RILL-R001: Type mismatch: cannot assign ${valueType} to $${name}:${explicitType}`, location, {
                    variableName: name,
            ...
            if (!this.ctx.variables.has(name) &&
                ...
                throw new RuntimeError('RILL-R001', `RILL-R001: Cannot reassign outer variable $${name} from child scope`, location, { variableName: name });
            }
            ...
            if (lockedType !== undefined && lockedType !== valueType) {
                throw new RuntimeError('RILL-R001', `RILL-R001: Type mismatch: cannot assign ${valueType} to $${name} (locked as ${lockedType})`, location, {
                    variableName: name,
        ...
        evaluateVariable(node) {
            ...
            if (node.isPipeVar && !node.name) {
                if (this.ctx.pipeValue === null) {
                    throw new RuntimeError('RILL-R005', 'RILL-R005: Undefined variable: $', this.getNodeLocation(node), { variable: '$' });
                }
            ...
            if (node.name) {
                ...
                if (result === undefined) {
                    throw new RuntimeError('RILL-R005', `RILL-R005: Undefined variable: $${node.name}`, this.getNodeLocation(node), { variable: node.name });
                }
                return result;
            }
            // Should not reach here - all variable nodes have either isPipeVar or name
            throw new RuntimeError('RILL-R005', 'RILL-R005: Invalid variable node', this.getNodeLocation(node));
        }
        /**
        ...
        async evaluateVariableAsync(node) {
            ...
            for (const access of node.accessChain) {
                if (value === null) {
                    ...
                    throw new RuntimeError('RILL-R009', `Cannot access property on null`, this.getNodeLocation(node));
                }
                ...
                if ('accessKind' in access) {
                    ...
                    if (Array.isArray(value)) {
                        if (typeof indexValue !== 'number') {
                            throw new RuntimeError('RILL-R002', `List index must be number, got ${inferType(indexValue)}`, this.getNodeLocation(node));
                        }
                        ...
                        if (result === undefined) {
                            throw new RuntimeError('RILL-R009', `List index out of bounds: ${indexValue}`, this.getNodeLocation(node));
                        }
                        value = result;
                    }
                    else if (isDict(value)) {
                        if (typeof indexValue !== 'string') {
                            throw new RuntimeError('RILL-R002', `Dict key must be string, got ${inferType(indexValue)}`, this.getNodeLocation(node));
                        }
                        const result = value[indexValue];
                        if (result === undefined) {
                            throw new RuntimeError('RILL-R009', `Undefined dict key: ${indexValue}`, this.getNodeLocation(node));
                        }
                        value = result;
                    }
                    else {
                        throw new RuntimeError('RILL-R002', `Cannot index ${inferType(value)}`, this.getNodeLocation(node));
                    }
                    continue;
                }
                ...
                if (access.kind === 'literal') {
                    ...
                    if (field === 'params') {
                        if (isCallable(value)) {
                            ...
                            if (node.defaultValue !== null) {
                                ...
                                throw new RuntimeError('RILL-R003', `Cannot access .params on ${inferType(value)}`, this.getNodeLocation(node), { actualType: inferType(value) });
                            }
                        }
                    }
                    else if (isDict(value)) {
                    ...
                    throw new RuntimeError('RILL-R002', `Field access kind '${access.kind}' not yet supported`, this.getNodeLocation(node));
                }
            }
            ...
            if (node.existenceCheck) {
                ...
                if (finalAccess.kind === 'variable') {
                    ...
                    if (keyValue === undefined) {
                        ...
                        throw new RuntimeError('RILL-R005', `RILL-R005: Variable '${varName}' is undefined`, this.getNodeLocation(node));
                    }
                    // Check if key exists in dict or list
                    if (isDict(value)) {
                        // EC-10: Key variable non-string
                        if (typeof keyValue !== 'string') {
                            throw new RuntimeError('RILL-R002', `Existence check key must be string, got ${inferType(keyValue)}`, this.getNodeLocation(node));
                        }
                ...
                if (finalAccess.kind === 'computed') {
                    ...
                    if (typeof keyValue !== 'string') {
                        throw new RuntimeError('RILL-R002', `Existence check key evaluated to ${inferType(keyValue)}, expected string`, this.getNodeLocation(node));
                    }
                ...
                throw new RuntimeError('RILL-R002', `Existence check not yet supported for ${finalAccess.kind} access`, this.getNodeLocation(node));
            }
        ...
        async evaluateFieldAccessVariable(access, value, node) {
            ...
            if (access.variableName === null) {
                ...
                keyValue = this.ctx.pipeValue ?? undefined;
                if (keyValue === undefined) {
                    throw new RuntimeError('RILL-R005', `RILL-R005: Pipe variable '$' is undefined`, this.getNodeLocation(node));
                }
            }
            else {
                ...
                if (keyValue === undefined) {
                    throw new RuntimeError('RILL-R005', `RILL-R005: Variable '${access.variableName}' is undefined`, this.getNodeLocation(node));
                }
            }
            // Validate key type (EC-2, EC-3)
            if (typeof keyValue === 'boolean') {
                throw new RuntimeError('RILL-R002', `Key must be string or number, got bool`, this.getNodeLocation(node));
            }
            if (Array.isArray(keyValue)) {
                throw new RuntimeError('RILL-R002', `Key must be string or number, got list`, this.getNodeLocation(node));
            }
            // Handle string key (dict access)
            if (typeof keyValue === 'string') {
                if (isDict(value)) {
                    // Allow missing fields to return null
                    return await this.accessDictField(value, keyValue, this.getNodeLocation(node), true);
                }
                return null;
            }
            ...
            throw new RuntimeError('RILL-R002', `Key must be string or number, got ${inferType(keyValue)}`, this.getNodeLocation(node));
        }
        /**
         ...
         */
        async evaluateFieldAccessComputed(access, value, node) {
            ...
            if (isCallable(keyValue)) {
                throw new RuntimeError('RILL-R002', `Computed key evaluated to closure, expected string or number`, this.getNodeLocation(node));
            }
            // EC-5: Expression result is dict
            if (isDict(keyValue)) {
                throw new RuntimeError('RILL-R002', `Computed key evaluated to dict, expected string or number`, this.getNodeLocation(node));
            }
            // Other invalid types (boolean, list)
            if (typeof keyValue === 'boolean') {
                throw new RuntimeError('RILL-R002', `Computed key evaluated to bool, expected string or number`, this.getNodeLocation(node));
            }
            if (Array.isArray(keyValue)) {
                throw new RuntimeError('RILL-R002', `Computed key evaluated to list, expected string or number`, this.getNodeLocation(node));
            }
            // Handle string key (dict access)
            if (typeof keyValue === 'string') {
                if (isDict(value)) {
            ...
            throw new RuntimeError('RILL-R002', `Computed key evaluated to unexpected type`, this.getNodeLocation(node));
        }
        /**
        ...
        async evaluateFieldAccessAlternatives(access, value, node) {
            ...
            if (!isDict(value)) {
                throw new RuntimeError('RILL-R002', `Alternative access requires dict, got ${inferType(value)}`, this.getNodeLocation(node));
            }
            // Try each alternative left-to-right (short-circuit on first match)
            for (const key of access.alternatives) {
                const dictValue = value[key];
