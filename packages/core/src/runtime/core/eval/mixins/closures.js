...
function createClosuresMixin(Base) {
    return class ClosuresEvaluator extends Base {
         ...
         */
        async invokeCallable(callable, args, callLocation, functionName) {
            this.checkAborted();
        ...
        createCallableContext(callable) {
            ...
            const hasExplicitParams = callable.params.length > 0 && callable.params[0].name !== '$';
            const callableCtx = {
                ...this.ctx,
        ...
        validateParamType(param, value, callLocation) {
            ...
            if (expectedType !== null) {
                ...
                if (valueType !== expectedType) {
                    throw new RuntimeError('RILL-R001', `Parameter type mismatch: ${param.name} expects ${expectedType}, got ${valueType}`, callLocation, { paramName: param.name, expectedType, actualType: valueType });
                }
            }
        }
        /**
        ...
        async evaluateHostCall(node) {
            ...
            if (args.length === 0 && this.ctx.pipeValue !== null) {
                const fnHasTypedZeroParams = typeof fn === 'object' &&
                    'params' in fn &&
                    fn.params !== undefined &&
                    fn.params.length === 0;
            ...
            const wrappedPromise = this.withTimeout((async () => {
                ...
                if (typeof fn === 'function') {
                    ...
                    return this.invokeCallable(callable, args, node.span.start, node.name);
                }
                else {
        ...
        async evaluateClosureCallWithPipe(node, pipeInput) {
            ...
            if (value === undefined || value === null) {
                throw new RuntimeError('RILL-R005', `Unknown variable: $${node.name}`, this.getNodeLocation(node), { variableName: node.name });
            }
            ...
            for (const prop of node.accessChain) {
                if (value === null) {
                    throw new RuntimeError('RILL-R009', `Cannot access property '${prop}' on null`, this.getNodeLocation(node));
                }
                if (isDict(value)) {
                    value = value[prop];
                    if (value === undefined || value === null) {
                        throw new RuntimeError('RILL-R009', `Dict has no field '${prop}'`, this.getNodeLocation(node));
                    }
                }
                else {
        ...
        async evaluatePipePropertyAccess(node, pipeInput) {
            ...
            for (const access of node.accessChain) {
                ...
                if ('accessKind' in access) {
                    ...
                    throw new RuntimeError('RILL-R002', 'Bracket access not supported in this context', this.getNodeLocation(node));
                }
                ...
                if (access.kind === 'literal') {
                    ...
                    throw new RuntimeError('RILL-R002', `Field access kind '${access.kind}' not supported in this context`, this.getNodeLocation(node));
                }
            }
        ...
        async evaluateVariableInvoke(node, _pipeInput) {
            ...
            throw new RuntimeError('RILL-R002', 'evaluateVariableInvoke is a placeholder - use evaluateVariableAsync from VariablesMixin', this.getNodeLocation(node));
        }
        /**
        ...
        async evaluateMethod(node, receiver) {
            ...
            if (isDict(receiver)) {
                ...
                if (dictValue !== undefined && isCallable(dictValue)) {
                    return this.invokeCallable(dictValue, args, this.getNodeLocation(node), node.name);
                }
            }
        ...
        async evaluateAnnotationAccess(value, key, location) {
            ...
            if (annotationValue === undefined) {
                throw new RuntimeError('RILL-R008', `Annotation '${key}' not found`, location, { annotationKey: key });
            }
         ...
         * Throws RUNTIME_TYPE_ERROR for non-closure targets.
         */
        async evaluateParamsProperty(callable, location) {
            // Only ScriptCallable supports .params reflection
            if (!isScriptCallable(callable)) {
                throw new RuntimeError('RILL-R003', `Cannot access .params on ${inferType(callable)}`, location, { actualType: inferType(callable) });
            }
