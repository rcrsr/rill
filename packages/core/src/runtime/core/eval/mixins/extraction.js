    ...
    return class ExtractionEvaluator extends Base {
        ...
        evaluateDestructure(node, input) {
            ...
            if (isKeyPattern) {
                ...
                if (!isDictInput) {
                    throw new RuntimeError('RILL-R002', `Key destructure requires dict, got ${isList ? 'list' : typeof input}`, node.span.start);
                }
                for (const elem of node.elements) {
                    if (elem.kind === 'skip')
                        continue;
                    if (elem.kind === 'nested') {
                        throw new RuntimeError('RILL-R002', 'Nested destructure not supported in dict patterns', elem.span.start);
                    }
                    if (elem.kind !== 'keyValue' ||
                        elem.key === null ||
                        elem.name === null) {
                        throw new RuntimeError('RILL-R002', 'Dict destructure requires key: $var patterns', elem.span.start);
                    }
                    const dictInput = input;
                    if (!(elem.key in dictInput)) {
                        throw new RuntimeError('RILL-R009', `Key '${elem.key}' not found in dict`, elem.span.start, { key: elem.key, availableKeys: Object.keys(dictInput) });
                    }
                    const dictValue = dictInput[elem.key];
                    if (dictValue === undefined) {
                        throw new RuntimeError('RILL-R009', `Key '${elem.key}' has undefined value`, elem.span.start);
                    }
                ...
                if (!isList) {
                    throw new RuntimeError('RILL-R002', `Positional destructure requires list, got ${isDictInput ? 'dict' : typeof input}`, node.span.start);
                }
                const listInput = input;
                if (node.elements.length !== listInput.length) {
                    throw new RuntimeError('RILL-R002', `Destructure pattern has ${node.elements.length} elements, list has ${listInput.length}`, node.span.start);
                }
                for (let i = 0; i < node.elements.length; i++) {
                    ...
                    if (elem.name === null) {
                        throw new RuntimeError('RILL-R002', 'Invalid destructure element', elem.span.start);
                    }
                    // Note: setVariable will be available from VariablesMixin
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        async evaluateSlice(node, input) {
            ...
            if (!isList && !isString) {
                throw new RuntimeError('RILL-R002', `Slice requires list or string, got ${isDict(input) ? 'dict' : typeof input}`, node.span.start);
            }
            ...
            switch (bound.type) {
                ...
                case 'Variable': {
                    ...
                    if (typeof value !== 'number') {
                        throw new RuntimeError('RILL-R002', `Slice bound must be number, got ${typeof value}`, bound.span.start);
                    }
                    return value;
                }
                case 'GroupedExpr': {
        ...
        applySlice(input, len, start, stop, step) {
            ...
            if (actualStep === 0) {
                throw new RuntimeError('RILL-R002', 'Slice step cannot be zero', undefined);
            }
