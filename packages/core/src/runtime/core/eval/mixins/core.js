...
function createCoreMixin(Base) {
    return class CoreEvaluator extends Base {
        ...
        async evaluatePostfixExpr(expr) {
            try {
                ...
                if (error instanceof RuntimeError &&
                    ...
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return this.evaluateBody(expr.defaultValue);
                }
            ...
            switch (primary.type) {
                ...
                case 'MethodCall':
                    if (this.ctx.pipeValue === null) {
                        throw new RuntimeError('RILL-R005', 'Undefined variable: $', primary.span?.start, { variable: '$' });
                    }
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ...
                case 'Block':
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return this.createBlockClosure(primary);
                case 'GroupedExpr':
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ...
                case 'Assert':
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return this.evaluateAssert(primary);
                case 'Error':
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return this.evaluateError(primary);
                case 'Pass':
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return this.evaluatePass(primary);
                case 'TypeAssertion': {
                    // Postfix type assertion: the operand is already evaluated
                    if (!primary.operand) {
                        throw new RuntimeError('RILL-R004', 'Postfix type assertion requires operand', primary.span.start);
                    }
                ...
                case 'TypeCheck': {
                    ...
                    if (!primary.operand) {
                        throw new RuntimeError('RILL-R004', 'Postfix type check requires operand', primary.span.start);
                    }
                ...
                default:
                    throw new RuntimeError('RILL-R004', `Unsupported expression type: ${primary.type}`, this.getNodeLocation(primary));
            }
        }
        /**
        ...
        async evaluatePipeTarget(target, input) {
            ...
            switch (target.type) {
                ...
                case 'Block': {
                    ...
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const closure = this.createBlockClosure(target);
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return this.invokeCallable(closure, [input], this.getNodeLocation(target));
                }
                case 'Closure': {
                    // Inline closure: create and invoke
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const closure = await this.createClosure(target);
                    ...
                    if (closure.params.length > 0) {
                        ...
                        try {
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            return await this.invokeCallable(closure, [], this.getNodeLocation(target));
                        }
                        finally {
                            this.ctx.pipeValue = savedPipeValue;
                        }
                ...
                case 'Dict': {
                    ...
                    if (Array.isArray(input) && !isTuple(input)) {
                        ...
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        const dictValue = await this.evaluateDict(target);
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        return await this.evaluateHierarchicalDispatch(dictValue, input, target.defaultValue, this.getNodeLocation(target));
                    }
                    // Dict dispatch: lookup key matching piped value
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return this.evaluateDictDispatch(target, input);
                }
                case 'Tuple': {
                    // Hierarchical dispatch: detect list input (not tuple)
                    if (Array.isArray(input) && !isTuple(input)) {
                        ...
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        const listValue = await this.evaluateTuple(target);
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        return await this.evaluateHierarchicalDispatch(listValue, input, target.defaultValue, this.getNodeLocation(target));
                    }
                    // Tuple dispatch: index lookup matching piped value
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return this.evaluateListDispatch(target, input);
                }
                case 'GroupedExpr':
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ...
                case 'Variable': {
                    ...
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const value = await this.evaluateVariableAsync(target);
                    ...
                    if (isCallable(value)) {
                        ...
                        if (hasParams) {
                            ...
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            return this.invokeCallable(value, [input], this.getNodeLocation(target));
                        }
                        else {
                            // Zero-param closure: invoke with args = [] and pipeValue = input
                            const savedPipeValue = this.ctx.pipeValue;
                            ...
                            try {
                                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                const result = await this.invokeCallable(value, [], this.getNodeLocation(target));
                                return result;
                            }
                            finally {
                                this.ctx.pipeValue = savedPipeValue;
                    ...
                    if (Array.isArray(input) && !isTuple(input)) {
                        if (isDict(value) || (Array.isArray(value) && !isTuple(value))) {
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            return await this.evaluateHierarchicalDispatch(value, input, target.defaultValue, this.getNodeLocation(target));
                        }
                    }
                    if (Array.isArray(value) && !isTuple(value)) {
                        // List dispatch
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        return await this.dispatchToList(value, input, target.defaultValue, target);
                    }
                    if (isDict(value)) {
                        // Dict dispatch
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        return await this.dispatchToDict(value, input, target.defaultValue, target);
                    }
                    ...
                    const valueType = typeof value === 'object' && value !== null
                        ? Array.isArray(value)
                            ...
                            : 'dict'
                        : typeof value;
                    throw RuntimeError.fromNode('RILL-R002', `Cannot dispatch to ${valueType}`, target);
                }
                case 'PostfixExpr': {
                ...
                case 'Assert':
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return this.evaluateAssert(target, input);
                case 'Error':
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return this.evaluateError(target, input);
                default:
                    throw new RuntimeError('RILL-R004', `Unsupported pipe target type: ${target.type}`, this.getNodeLocation(target));
            }
        }
        /**
         ...
         */
        async evaluateHierarchicalDispatch(target, path, defaultExpr, location) {
            ...
            try {
                ...
                for (let i = 0; i < path.length - 1; i++) {
                    const key = path[i];
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    current = await this.traversePathStep(current, key, false, location);
                }
                // Handle last element separately for terminal closure support
                lastKey = path[path.length - 1];
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const result = await this.traversePathStep(current, lastKey, true, location);
                // Resolve terminal value (handles terminal closures with $ = lastKey)
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                return await this.resolveTerminalValue(result, lastKey, location);
            }
            catch (error) {
                ...
                if (error instanceof RuntimeError && error.errorId === 'RILL-R009') {
                    if (defaultExpr) {
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        return await this.evaluateExpression(defaultExpr);
                    }
         ...
         */
        async traversePathStep(current, key, isTerminal, location) {
            ...
            if (isDict(current) && typeof key === 'string') {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const result = await this.dispatchToDict(current, key, null, // No default value for intermediate steps
                locObj, true // Skip closure resolution - we handle it here
                );
                // Non-terminal closures must be resolved via resolveIntermediateClosure
                if (!isTerminal && isCallable(result)) {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    return await this.resolveIntermediateClosure(result, location);
                }
            ...
            if (Array.isArray(current) &&
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const result = await this.dispatchToList(current, key, null, // No default value for intermediate steps
                locObj, true // Skip closure resolution - we handle it here
                );
                // Non-terminal closures must be resolved via resolveIntermediateClosure
                if (!isTerminal && isCallable(result)) {
            ...
            throw new RuntimeError('RILL-R002', `Hierarchical dispatch type mismatch: cannot use ${keyType} key with ${currentType} value`, location, { currentType, keyType, key });
        }
        /**
         ...
         */
        async resolveIntermediateClosure(value, location) {
            if (!isCallable(value)) {
                return value;
            }
            ...
            if (value.kind === 'script' && value.params.length >= 1) {
                ...
                if (value.params[0].name !== '$') {
                    ...
                    throw new RuntimeError('RILL-R002', 'Cannot invoke parameterized closure at intermediate path position', location);
                }
            }
            // Zero-param closure or block-closure: auto-invoke with args = []
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return await this.invokeCallable(value, [], location);
        }
        /**
         ...
         * @throws RuntimeError with RUNTIME_TYPE_ERROR if parameterized closure
         */
        async resolveTerminalValue(value, finalKey, location) {
            if (!isCallable(value)) {
                return value;
            }
            // Check for parameterized closure (explicit user-defined params)
            ...
            if (value.kind === 'script' && value.params.length >= 1) {
                ...
                if (value.params[0].name !== '$') {
                    ...
                    throw new RuntimeError('RILL-R002', 'Dispatch does not provide arguments for parameterized closure', location);
                }
            }
            // Check if callable has params to determine invocation style
            const hasParams = (value.kind === 'script' && value.params.length > 0) ||
            ...
            if (hasParams) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                return await this.invokeCallable(value, [finalKey], location);
            }
            else {
                ...
                try {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const result = await this.invokeCallable(value, [], location);
                    return result;
                }
                finally {
                    this.ctx.pipeValue = savedPipeValue;
