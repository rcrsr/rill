...
import { executeStatement, checkAutoExceptions, checkAborted, } from './eval/index.js';
        ...
        async step() {
            ...
            try {
                ...
                if (isRecoveryErrorNode(stmt)) {
                    throw RuntimeError.fromNode('RILL-P002', `Cannot execute RecoveryErrorNode: ${stmt.message}. Use parse() instead of parseWithRecovery() for execution.`, stmt);
                }
        ...
        getResult() {
            ...
            if (total === 0) {
                if (context.pipeValue === null) {
                    throw new RuntimeError('RILL-R005', 'Undefined variable: $', undefined, { variable: '$' });
                }
                return {
 ...
 * @internal
 */
function isRecoveryErrorNode(stmt) {
