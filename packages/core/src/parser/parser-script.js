            ...
            try {
                ...
                if (err instanceof ParseError || err instanceof LexerError) {
                    ...
                    const parseError = err instanceof ParseError
                        ? err
                        : new ParseError('RILL-P001', err.message.replace(/ at line \d+, column \d+$/, ''), err.location);
                    this.state.errors.push(parseError);
                    // Create RecoveryErrorNode and skip to next statement boundary
                    const errorNode = this.recoverToNextStatement(stmtStart, parseError.message);
...
Parser.prototype.parseFrontmatter = function () {
    const start = current(this.state).span.start;
    const openingDelim = expect(this.state, TOKEN_TYPES.FRONTMATTER_DELIM, 'Expected ---');
