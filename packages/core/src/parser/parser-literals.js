...
import { isDictStart, FUNC_PARAM_TYPES, parseTypeName, isNegativeNumber, } from './helpers.js';
// ============================================================
// LITERAL PARSING
// ============================================================
Parser.prototype.parseLiteral = function () {
    ...
    throw new ParseError('RILL-P001', `Expected literal, got: ${token.value}${hint}`, token.span.start);
};
// ============================================================
// STRING PARSING
// ============================================================
...
Parser.prototype.parseStringParts = function (raw, baseLocation, isTokenMultiline) {
    const parts = [];
    let i = 0;
    let literalStart = 0;
    while (i < raw.length) {
        if (raw[i] === '{') {
            ...
            if (depth !== 0) {
                throw new ParseError('RILL-P005', 'Unterminated string interpolation', baseLocation);
            }
            const exprSource = raw.slice(exprStart, i - 1);
            if (!exprSource.trim()) {
                throw new ParseError('RILL-P004', 'Empty string interpolation', baseLocation);
            }
...
Parser.prototype.parseInterpolationExpr = function (source, baseLocation) {
    ...
    if (subParser.state.tokens[subParser.state.pos]?.type !== TOKEN_TYPES.EOF) {
        throw new ParseError('RILL-P001', `Unexpected token in interpolation: ${subParser.state.tokens[subParser.state.pos]?.value}`, baseLocation);
    }
    return {
        type: 'Interpolation',
        expression,
...
Parser.prototype.parseTupleElement = function () {
    ...
    if (check(this.state, TOKEN_TYPES.ELLIPSIS)) {
        ...
        if (check(this.state, TOKEN_TYPES.COMMA) ||
            ...
            throw new ParseError('RILL-P004', "Expected expression after '...'", current(this.state).span.start);
        }
...
Parser.prototype.parseDictEntry = function () {
    ...
    if (check(this.state, TOKEN_TYPES.DOLLAR)) {
        ...
        if (!check(this.state, TOKEN_TYPES.IDENTIFIER)) {
            throw new ParseError('RILL-P005', 'Expected variable name after $', current(this.state).span.start);
        }
        ...
        if (!check(this.state, TOKEN_TYPES.RPAREN)) {
            throw new ParseError('RILL-P005', 'Expected ) after computed key expression', current(this.state).span.start);
        }
        advance(this.state); // consume )
        key = {
            kind: 'computed',
        ...
        if (literal.type !== 'Tuple') {
            throw new ParseError('RILL-P004', 'Dict entry key must be identifier or list, not dict', literal.span.start);
        }
        key = literal;
    }
        ...
        throw new ParseError('RILL-P001', 'Dict key must be identifier, string, number, boolean, variable, or expression', current(this.state).span.start);
    }
