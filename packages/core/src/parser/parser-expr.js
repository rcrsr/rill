...
import { check, advance, expect, current, makeSpan, peek, skipNewlines, } from './state.js';
...
Parser.prototype.parseCommonConstruct = function () {
    ...
    if (check(this.state, TOKEN_TYPES.BANG)) {
        ...
        if (check(this.state, TOKEN_TYPES.EOF, TOKEN_TYPES.NEWLINE, TOKEN_TYPES.RPAREN)) {
            throw new ParseError('RILL-P004', 'Negation operator requires an operand. Use prefix syntax: !expr or (!expr)', start);
        }
...
Parser.prototype.parsePrimary = function () {
    ...
    if (token.type === TOKEN_TYPES.LT &&
        ...
        throw new ParseError('RILL-P001', `Unexpected token: ${token.value}. Hint: Heredoc syntax (<<EOF) was removed, use triple-quote strings (""") instead`, token.span.start);
    }
    throw new ParseError('RILL-P001', `Unexpected token: ${token.value}`, token.span.start);
};
// ============================================================
// PIPE TARGET PARSING
// ============================================================
Parser.prototype.parsePipeTarget = function () {
    ...
    throw new ParseError('RILL-P001', `Expected pipe target, got: ${current(this.state).value}`, current(this.state).span.start);
};
// ============================================================
// CAPTURE PARSING
// ============================================================
