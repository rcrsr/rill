...
Parser.prototype.parseConditionalWithCondition = function (conditionBody) {
...
// LOOPS
// ============================================================
Parser.prototype.parseLoop = function (condition) {
...
Parser.prototype.parseLoopWithInput = function (condition) {
    let conditionExpr;
    if (condition.type === 'PipeChain') {
        ...
        conditionExpr = {
            type: 'PipeChain',
            head: condition.type === 'PostfixExpr'
                ? condition
                : {
                    type: 'PostfixExpr',
                    primary: condition,
...
// ERROR
// ============================================================
Parser.prototype.parseError = function (requireMessage = false) {
    const start = current(this.state).span.start;
    ...
    if (check(this.state, TOKEN_TYPES.STRING)) {
        ...
        const atBoundary = isAtEnd(this.state) ||
