...
Parser.prototype.makeVariableWithAccess = function (name, isPipeVar, start) {
    ...
    if (check(this.state, TOKEN_TYPES.NULLISH_COALESCE)) {
        if (existenceCheck) {
            ...
            throw new ParseError('RILL-P003', 'Cannot combine existence check (.?field) with default value operator (??). Use one or the other.', token.span.start);
        }
        advance(this.state);
...
Parser.prototype.parseAccessChain = function () {
    ...
    while (check(this.state, TOKEN_TYPES.DOT, TOKEN_TYPES.DOT_QUESTION, TOKEN_TYPES.LBRACKET)) {
        ...
        if (check(this.state, TOKEN_TYPES.LBRACKET)) {
            ...
            const closeBracket = expect(this.state, TOKEN_TYPES.RBRACKET, 'Expected ] after index expression');
...
Parser.prototype.parseFieldAccessElement = function (isExistenceCheck = false) {
    ...
    if (check(this.state, TOKEN_TYPES.CARET)) {
        advance(this.state);
        const keyToken = expect(this.state, TOKEN_TYPES.IDENTIFIER, 'Expected annotation key after .^');
...
Parser.prototype.parseComputedOrAlternatives = function (isExistenceCheck = false) {
    advance(this.state);
