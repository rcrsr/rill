...
describe('Rill Runtime: List Membership Methods', () => {
  describe('.has()', () => {
    ...
    describe('edge cases', () => {
      it('finds string in string list', async () => {
        expect(await run('["one", "two", "three"] -> .has("two")')).toBe(
          true
        );
      });
  ...
  describe('.has_any()', () => {
    describe('basic functionality', () => {
      ...
      it('returns true when first candidate matches', async () => {
        expect(await run('[1, 2, 3] -> .has_any([1, 99, 100])')).toBe(
          true
        );
      });

      it('returns true when last candidate matches', async () => {
        expect(await run('[1, 2, 3] -> .has_any([99, 100, 3])')).toBe(
          true
        );
      });
    });
    ...
    describe('type safety', () => {
      it('respects type differences', async () => {
        expect(await run('[1, 2, 3] -> .has_any(["1", "2"])')).toBe(
          false
        );
      });

      it('finds matching types', async () => {
        expect(await run('[1, 2, 3] -> .has_any([4, 2, 5])')).toBe(
          true
        );
      });
    });
    ...
    describe('edge cases', () => {
      it('handles homogeneous candidates', async () => {
        expect(await run('[1, 2, 3] -> .has_any([5, 2, 6])')).toBe(
          true
        );
      });
      ...
      it('handles duplicate candidates', async () => {
        expect(await run('[1, 2, 3] -> .has_any([2, 2, 2])')).toBe(
          true
        );
      });
    });
  ...
  describe('.has_all()', () => {
    describe('basic functionality', () => {
      ...
      it('returns false when any candidate missing', async () => {
        expect(await run('[1, 2, 3] -> .has_all([1, 2, 4])')).toBe(
          false
        );
      });
      ...
      it('returns true when all candidates present', async () => {
        expect(
          await run('[1, 2, 3, 4, 5] -> .has_all([1, 3, 5])')
        ).toBe(true);
      });
    ...
    describe('edge cases', () => {
      ...
      it('handles duplicate candidates', async () => {
        expect(await run('[1, 2, 3] -> .has_all([2, 2, 2])')).toBe(
          true
        );
      });
  ...
  describe('integration with other features', () => {
    it('works in conditional expressions', async () => {
      expect(
        await run('[1, 2, 3] -> .has(2) ? "found" ! "not found"')
      ).toBe('found');
    });
