...
// LOGGING
// ============================================================
const LOG_PRIORITY = { silent: 0, info: 1, debug: 2 };
function log(level, msg, logLevel) {
    if (LOG_PRIORITY[level] <= LOG_PRIORITY[logLevel]) {
        console.log(msg);
    }
}
// eslint-disable-next-line no-redeclare
export function createAgentHost(agentOrAgents, options) {
    // ----------------------------------------------------------
    ...
    const cfg = {
        ...
        maxConcurrentSessions: options?.maxConcurrentSessions ?? DEFAULTS.maxConcurrentSessions,
    ...
    function pushSseEvent(sessionId, event, data) {
     ...
     */
    async function runForAgent(input, composedAgent) {
        if (phase === 'stopped') {
        ...
        log('debug', `[host] session ${sessionId} started (trigger: ${input.trigger ?? 'api'})`, cfg.logLevel);
        ...
        let resolved = false;
        let timeoutHandle;
        const executePromise = execute(composedAgent.ast, sessionContext).then((result) => {
    ...
    const host = {
        ...
        async run(input) {
            ...
            const composedAgent = agentsMap.values().next().value;
        ...
        // ----------------------------------------------------------
        async runForAgent(agentName, input) {
        ...
        async listen(port) {
            ...
            for (const [agentName, composedAgent] of agentsMap) {
                ...
                registerRoutes(agentApp, agentRouteHost, composedAgent.card, sseStore, composedAgent.card.input);
            ...
            const agentNames = Array.from(agentsMap.keys()).join(', ');
            log('info', `[host] ${agentNames} listening on http://localhost:${listenPort}`, cfg.logLevel);
            ...
            if (registryUrl !== undefined && registryUrl !== '') {
                ...
                const ahiDependencies = Array.isArray(ahiAgents) &&
                    ahiAgents.every((a) => typeof a === 'string')
                    ? ahiAgents
                    : [];
                const { createRegistryClient } = await import('@rcrsr/rill-registry-client');
 ...
 */
function registerProcessRoutes(app, host) {
    app.get('/healthz', (c) => {
 ...
 */
async function deliverCallback(callbackUrl, response, record) {
