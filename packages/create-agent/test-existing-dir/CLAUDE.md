...
|-----------|---------|---------------|
| `anthropic` | `@rcrsr/rill-ext-anthropic` | [Extension Docs](https://www.npmjs.com/package/@rcrsr/rill-ext-anthropic) |
...
```bash
npm start      # Run the agent script
npm run check  # Validate rill script syntax
```

Use `npm start` to execute your agent. Edit `src/agent.rill` to modify agent behavior and `src/host.ts` to add custom host functions.