...
// Directory of this test file — has access to packages/compose/node_modules
const THIS_DIR = dirname(fileURLToPath(import.meta.url));

// The compose package root has esbuild/zod in node_modules
const COMPOSE_DIR = join(THIS_DIR, '..');
 ...
 * Used to trigger EC-18 (no factory export).
 */
...
describe('resolveExtensions', () => {
  ...
  describe('npm strategy', () => {
    it('populates resolvedVersion for a successfully resolved npm package', async () => {
      // Create a fake extension package directly in packages/compose/node_modules/
  ...
  describe('error conditions', () => {
    describe('EC-15: npm package not found [AC-14]', () => {
    ...
    describe('EC-16: local path not found', () => {
      ...
      it('throws ComposeError for path traversal outside manifestDir', async () => {
    ...
    describe('EC-18: no factory export [AC-20]', () => {
  ...
  describe('built-in strategy [AC-9]', () => {
    it('throws EC-17 for invalid built-in name before attempting import', async () => {
      // Name validation runs before any dynamic import, so this test is
      // unaffected by the Vite alias constraint described below.
