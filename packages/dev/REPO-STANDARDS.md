...
Elements needing judgement land in the third state, and so does a
cross-repository comparison the checker cannot resolve — but not every
cross-repository comparison is unresolvable. `@rcrsr/rill-dev` ships a
baseline generated from rill's own tree (`packages/dev/baseline.json`,
generated by `rill-gen-baseline`) alongside the checker, so a handful of
elements that used to be structurally undecidable from a single checkout —
STD-LINT-1, STD-LINT-5, STD-LINT-9, STD-PM-2, STD-DEP-1, STD-DEP-2, STD-DEP-5 —
are checked against it instead of skipped. A repository running a
`@rcrsr/rill-dev` old enough not to carry the baseline still sees these as `--`,
named as such. The script prints the `--` count in the summary rather than
omitting it, because a checker that silently passes what it cannot decide is
worse than no checker: it converts an unknown into a false assurance. An
unreachable API degrades to `--` as well, never to `FAIL`.
...
**STD-LINT-3's check reads the actual override glob, not just the rule's
severity.** These config files carry comments and so are not
`JSON.parse`-able directly; the checker strips them with a small JSONC reader
(`packages/dev/jsonc.cjs`) rather than grepping. It confirms the plugin is
loaded, walks `overrides[]` for the entry that sets
`rill/no-spec-id-reference` to `error`, and requires that entry's own `files`
glob to name `src`. A rule enabled at `error` under an override scoped
somewhere else now reads as non-conformant rather than passing on the rule's
presence alone.
...
A consuming repository cannot read those two files directly, so the pins they
