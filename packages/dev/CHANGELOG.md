...
- **Baseline refreshed to the current shared-tooling pins:** `baseline.json` now records `@types/node ^26.5.0`, `knip ^6.35.0`, `lefthook ^2.1.12`, `oxfmt ^0.67.0`, `oxlint ^1.82.0`, and `vitest ^5.0.0`, and its `packageManager` pin moves from `pnpm@11.18.0` to `pnpm@12.3.4`, matching rill's tree after a dev-dependency sweep. Consumers reading `@rcrsr/rill-dev/baseline.json` resolve `STD-DEP-1` and `STD-PM-2` against the same ranges rill pins today. Both the `packageManager` pin and the `vitest` range cross a major. A consumer still on pnpm 11 fails `STD-PM-2` until it runs `corepack use pnpm@12.3.4`; pnpm 12 rejects unrecognized `pnpm-workspace.yaml` keys with `ERR_PNPM_UNRECOGNIZED_WORKSPACE_SETTINGS` once the pin is honored, so a misspelled setting that was silently ignored under 11 now fails the install. For vitest, vitest 5 no longer searches parent directories for a config file, so a consumer that runs `vitest run` from a package directory and relies on a root `vitest.config.ts` needs a per-package config that re-exports it. No checker logic changed. ([#448](https://github.com/rcrsr/rill/pull/448))
