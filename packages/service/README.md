...
`runRules` defaults to `RULES`; pass a rule array to run a subset. `config` is a `CheckConfig`, whose `rules` field holds per-rule state and whose optional `checkerMode` selects `'strict'` or `'permissive'`. The registry holds 41 rules.
