 ...
 *
 * This test reads every `*.test.ts` file under src/rules/ as raw text and
 * flags each `runRules(...)` call whose final argument is a bare `[]`.
 * One legitimate probe (an explicit "zero rules produces zero diagnostics"
 * case) is exempted by a per-file COUNT allowlist rather than a whole-file
 * skip, so a second offending call added anywhere - including in the
 * allowlisted file - still fails this test.
 */
...
describe('disabled rules guard: raw-text proof', () => {
  it('does not call runRules(...) with a bare empty rules array beyond the allowlisted count', () => {
    ...
    for (const fileName of listRuleTestFiles()) {
      ...
      const matches = [...contents.matchAll(EMPTY_RULES_CALL)].map(
        (match) => match[0]
      );
