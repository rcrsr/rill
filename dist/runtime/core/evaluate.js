...
function setVariable(ctx, name, value, explicitType, location) {
    ...
    if (!ctx.variables.has(name) && ctx.parent && hasVariable(ctx.parent, name)) {
        throw new RuntimeError(RILL_ERROR_CODES.RUNTIME_TYPE_ERROR, `Cannot shadow outer variable $${name} in child scope`, location, { variableName: name });
    }
...
function getBaseVariableValue(node, ctx) {
    ...
    if (node.name)
        return getVariable(ctx, node.name) ?? null;
    return null;
}
 ...
 * Each iteration creates a child scope (reads parent, writes local only).
 */
async function evaluateWhileLoopMode(node, initialCondition, ctx) {
...
async function evaluateBlock(node, ctx) {
    ...
    let lastValue = childCtx.pipeValue;
