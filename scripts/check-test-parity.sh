...
#   1. No test file under packages/core/tests/ differs from the base ref
#      (merge-base with main by default). Tests are the arbiter; the branch
#      must never touch them.
#   2. No untracked files inside packages/core/tests/ (a new test file would
#      change the arbiter just as much as an edit).
...
BASE_REF="${1:-main}"
BASELINE_TESTS=5752 # recorded 2026-07-15 on main @ 20ebc8c
...
if ! git diff --quiet "$MERGE_BASE" -- packages/core/tests/; then
  echo "FAIL: test files differ from ${BASE_REF}:"
  git diff --stat "$MERGE_BASE" -- packages/core/tests/
  exit 1
fi

# 2. No untracked files may appear inside the test tree.
UNTRACKED=$(git ls-files --others --exclude-standard packages/core/tests/)
if [ -n "$UNTRACKED" ]; then
  echo "FAIL: untracked files inside packages/core/tests/:"
  echo "$UNTRACKED"
  exit 1
fi
echo "OK: packages/core/tests/ identical to ${BASE_REF}"
